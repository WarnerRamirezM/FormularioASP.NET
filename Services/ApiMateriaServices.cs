
using Newtonsoft.Json;
using WebApplication1.Models.ApiModels;

namespace WebApplication1.Services
{
    public class ApiMateriaServices
    {
        private readonly HttpClient _client;

        // Constructor con HttpClient inyectado
        public ApiMateriaServices(HttpClient client)
        {
            _client = client;
        }
        public async Task<List<MateriaModel>> ObtenerMateriasAsync()
        {
            // Realiza una petición GET al endpoint del API para obtener las materias
            var response = await _client.GetAsync("api/Materias");

            // Verifica si la respuesta fue exitosa (código 200 OK)
            if (response.IsSuccessStatusCode)
            {
                // Lee el contenido de la respuesta como un string JSON
                var json = await response.Content.ReadAsStringAsync();

                // Deserializa el JSON a una lista de objetos MateriaModel
                var materias = JsonConvert.DeserializeObject<List<MateriaModel>>(json);

                // Verifica si la deserialización no devolvió null
                if (materias != null)
                    return materias; // Retorna la lista deserializada
                else
                    return new List<MateriaModel>(); // Si fue null, retorna una lista vacía
            }

            // Si la respuesta no fue exitosa, retorna una lista vacía
            return new List<MateriaModel>();
        }

    }
}
